...
-- Khadgar: dedicated expedition master cloned from stock Khadgar.
...
-- Reward chest template. It is not permanently spawned in Goldshire; Gauntlet
-- may use this entry dynamically for dungeon rewards.
...
SET
    `entry` = @GAUNTLET_REWARD_CHEST_ENTRY,
...
-- Account bank template. The C++ bank system summons it next to Khadgar.
...
-- Lobo solitario. Keep the complete server SpellMgr row here; the client DBC
-- remains authored by the Gauntlet client pipeline.
