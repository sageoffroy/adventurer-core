...
constexpr char BookPrefix[] = "AGBOOK|";
...
{
    return entry >= GauntletItemMin && entry <= GauntletItemMax && sObjectMgr->GetItemTemplate(entry);
}
...
void DiscoverItem(Player* player, uint32 entry)
{
    ...
    CharacterDatabase.DirectExecute(
        "INSERT IGNORE INTO `adventurer_gauntlet_account_collection` (`account_id`, `item_entry`) VALUES ({}, {})",
        accountId,
        entry);
}

void SendBookState(Player* player)
{
    uint32 accountId = GetAccountId(player);
    if (!accountId)
        return;
...
bool HandleBookCommand(Player* player, std::string const& rawMessage)
{
    if (!player)
        return false;
    ...
    else if (rawMessage.rfind(BookPrefix, 0) == 0)
        payload = rawMessage.substr(sizeof(BookPrefix) - 1);
    else
        return false;

    if (payload == "REFRESH")
    {
        SendBookState(player);
        return true;
    }
