...
# Importing adventurer_apply extends the normal client pipeline with the current
# Adventurer Item.dbc handling before this v3 layer wraps it.
...
def patch_spell_icon(path: Path, icons: list[tuple[Path, str, str]]) -> dict[str, int]:
    ...
    # Lobo solitario owns the first custom ID so Gauntlet can reference it
    # without a second generated manifest. If the filename is absent, 910000
    # simply remains unused until that icon is added to the pack.
    ordered = sorted(
        icons,
