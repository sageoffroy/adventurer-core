...
def patch_talent_tabs(path: Path, spec: dict) -> bool:
    ...
    source_rows = {int(tab["source_tab_id"]): bytearray(record_by_id(dbc, int(tab["source_tab_id"]), "TalentTab.dbc")) for tab in spec["tabs"]}
...
def patch_talents_and_spells(talent_path: Path, spell_path: Path, spec: dict) -> tuple[bool, bool]:
    ...
    # Source rows remain stock and are never modified. Custom rows are rebuilt on
    # every run, making the transform deterministic and idempotent.
    source_talents = {
    ...
    for index, definition in enumerate(talent_defs):
        ...
        new_talent_id = custom_talent_id(spec, index)
        set_u32(source, 0, new_talent_id)
