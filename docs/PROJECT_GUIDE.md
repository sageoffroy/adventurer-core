...
- `update.sh` — updates an existing Adventurer installation using the same existing pipeline and preserves editable runtime files where the tooling supports that behavior.
- `rollback.sh` — reverses Adventurer-owned installation state using the repository's current rollback mechanism.
- `preflight.sh` — read-only compatibility/preflight entrypoint for the existing installer.
...
Configuration and source data consumed by the build/install/runtime tooling.

#### `config/spelldraft/`

- `cards.csv` — authoritative packaged SpellDraft card catalogue. Defines cards, types, source levels, rarities, weights, rank grants and dependency metadata used by the runtime generation path.
...
- `subclasses.json` — presentation-subclass metadata used by the current SpellDraft/subclass tooling.
- `icy_touch.json` — reviewed source data for the current Icy Touch adaptation; consumed by the DK adaptation/generation path rather than being a generic hot-reload runtime file.

#### `config/playerbots/`

- `managed.conf` — Adventurer-managed Playerbots configuration/profile used when Playerbots integration is present.
...
- `AdventurerSpellTooltips.lua` — Adventurer spell tooltip augmentation; currently used by spell adaptations such as Icy Touch.
...
#### `payload/core/src/server/game/Spells/`

- `AdventurerSpellScaling.h` — generated/shared spell-scaling data/helper used by current native spell adaptations. Icy Touch generation currently feeds this path.
...
- `005_adventurer_chassis_80.sql` — current level-80-era chassis/world-data adjustments.

Treat the numbered files as historical migration/install inputs; inspect current database tooling before adding another SQL file.
...
- `core_patch.py` — exact-anchor transformations applied to native AzerothCore source files. This is the existing path for modifying upstream C++ files such as class support and `SpellInfo.cpp` behavior.
...
- `dk_adaptations.py` — DK-specific adaptation logic. Inspect this first for DK spell work.
...
- `spells/` — approved per-spell implementation contracts/checklists.
...
requested gameplay behavior
  ...
  -> reuse payload/shared helpers if appropriate
  -> apply/update
  -> rebuild worldserver
  -> start server
...
source config/catalogue
  -> tools/dbc.py and/or feature-specific generator
  -> staged server DBC
...
config/spelldraft/cards.csv + spelldraft.conf + metadata
  ...
  -> selected card teaches/progresses the configured spell/talent grants
```
...
existing native spell + desired Adventurer behavior
  ...
  -> feature-specific adapter (for DK: tools/dk_adaptations.py) if needed
...
See `SPELL_WORKFLOW.md`. Icy Touch is documented in `spells/ICY_TOUCH.md` and is the current example of a DK spell requiring catalogue + DBC/config-derived data + native `SpellInfo.cpp` behavior + client tooltip support.
...
Playerbots is optional and must not become a requirement for the core Adventurer installation.
...
Detailed safeguards and exact DB behavior live in `ROLLBACK.md`. Spell work should not add new rollback layers by default.
...
When architecture changes, update this guide. When SpellDraft rules change, update `SPELLDRAFT.md`. When a spell's approved behavior changes, update its file under `docs/spells/`. Avoid creating a new Markdown file for every development step or temporary experiment; Git history already preserves history.
