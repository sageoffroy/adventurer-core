...
local TECHNICAL_CLASS_ID = 1; -- Warrior; hidden from the player.

local function HideClassSelectionUI()
...
local function SelectTechnicalClassForCurrentRace()
	...
	local classId = TECHNICAL_CLASS_ID;

	-- Blood Elves cannot be Warriors in stock 3.3.5. Until CharBaseInfo.dbc is
	-- patched, fall back to the first valid hidden class so creation still works.
	if ( not IsRaceClassValid(raceId, classId) ) then
		for i=1, CharacterCreate.numClasses, 1 do
			if ( IsRaceClassValid(raceId, i) ) then
				classId = i;
				break;
			end
		end
...
function CharacterCreate_OnShow()
	...
	if ( PAID_SERVICE_TYPE ) then
		...
		SelectTechnicalClassForCurrentRace();
	end
...
function CharacterRace_OnClick(self, id)
	...
	if ( GetSelectedRace() ~= id ) then
		...
		CharacterCreateEnumerateClasses(GetAvailableClasses());
		if ( PAID_SERVICE_TYPE ) then
			SetCharacterClass(PaidChange_GetCurrentClassIndex());
		else
			SelectTechnicalClassForCurrentRace();
		end
